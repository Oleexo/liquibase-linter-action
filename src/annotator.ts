...
import type { AnnotationOptions, AnnotationResult, SeverityLevel } from './types.js';

/**
 * Creates GitHub Check Run with annotations for violations
 */
export async function createAnnotations(token: string, options: AnnotationOptions): Promise<AnnotationResult> {
  ...
  try {
    ...
    throw new Error(`Failed to create GitHub annotations: ${error instanceof Error ? error.message : String(error)}`);
  }
}
