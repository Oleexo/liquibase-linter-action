...
export async function installLinter(version: string): Promise<string> {
  try {
    ...
    const toolDir = path.join(process.env['RUNNER_TEMP'] || '/tmp', `liquibase-linter-${Date.now()}`);
    ...
    try {
      ...
      try {
        ...
        core.warning(
          `Binary verification skipped: unable to verify with --version or --help. The binary will be used as-is. Original error: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
    ...
    throw new Error(`Failed to install liquibase-linter: ${error instanceof Error ? error.message : String(error)}`);
  }
}
