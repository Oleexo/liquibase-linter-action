...
import { createAnnotations } from './annotator.js';
import { getGitHubContext } from './github.js';
import { installLinter } from './installer.js';
import { runLinter, validateConfigFile, validateDirectory } from './runner.js';
import type { LinterConfig, SeverityLevel } from './types.js';

/**
 * Main logic for the GitHub Action (testable, accepts context as params)
 */
export async function run(): Promise<void> {
    ...
    const severityThreshold = (core.getInput('severity-threshold', { required: false }) || 'info') as SeverityLevel;
    ...
    const workingDirectory = core.getInput('working-directory', { required: false }) || process.cwd();
    ...
    if (shouldAnnotate && results.summary.total_violations > 0) {
        ...
        try {
            ...
            core.info(`Repository: ${ghContext.owner}/${ghContext.repo}`);
            core.info(`SHA: ${ghContext.sha}`);
            ...
            core.warning(`Failed to create annotations: ${error instanceof Error ? error.message : String(error)}`);
